...
public class AccountManagerImpl implements AccountManager{
    ...
    public void updateAccount(Account account) throws ServiceFailureException{
        ...
        try (Connection conn = dataSource.getConnection()) {
            try(PreparedStatement st = conn.prepareStatement("UPDATE account SET name=?,description=?,creationdate=?,canceldate=? WHERE id=?")) {
                st.setString(1, account.getName() );
                st.setString(2, account.getDescription());
                st.setDate(3, java.sql.Date.valueOf(account.getCreationDate()));
                if(account.getCancelDate() != null)
                    st.setDate(4, java.sql.Date.valueOf(account.getCancelDate()));
                else
                    st.setDate(4, null);
    ...
    try (Connection conn = dataSource.getConnection()) {
            try (PreparedStatement st = conn.prepareStatement("SELECT id, name, description, creationdate, canceldate FROM account WHERE id = ?")) {
                st.setLong(1, id);
    ...
    private Account resultSetToAccount(ResultSet rs) throws SQLException {
        ...
        if(rs.getDate("canceldate") != null)
            account.setCancelDate(rs.getDate("canceldate").toLocalDate());
        return account;
    }
    ...
    public List<Account> findAllAccounts() throws ServiceFailureException{
        ...
        try (Connection conn = dataSource.getConnection()) {
            try (PreparedStatement st = conn.prepareStatement("SELECT id,name,description,creationdate,canceldate FROM account")) {
                ResultSet rs = st.executeQuery();
    ...
    public void createDatabase() throws SQLException {                
        ...
        bds.setUrl("jdbc:derby://localhost:1527/expenseDatabase");
